...
function FirebaseSettingsPanel() {
  ...
  function saveFirebaseConfig() {
    ...
    setStatus("firebase");
    setMessage("Firebase 설정을 저장했습니다. 실시간 DB를 사용합니다.");
  }
      ...
      {isOpen ? (
        <div className="mt-4 rounded-xl border border-[var(--color-border)] bg-white p-4">
          ...
          <textarea
            ...
            className="mt-3 h-48 w-full rounded-lg border border-[var(--color-border)] bg-white p-3 font-mono text-sm font-bold leading-5 text-[var(--color-text)] outline-none focus:border-[var(--color-brand)] focus:ring-4 focus:ring-blue-100"
            spellCheck={false}
          />
...
function PinDigitRow({ value, length, onChange, accent = "blue", autoFocus = false, masked = false }) {

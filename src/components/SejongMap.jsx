...
function getDistrictStyle(feature, assignments, selectedAreaIds, selectableAreaIds) {
  ...
  if (isSelected) {
    return {
      fill: ACTIVE_SELECTION.fill,
      stroke: ACTIVE_SELECTION.stroke,
      strokeWidth: 3.6,
      opacity: 1,
    };
  }
...
function VoteCallout({ anchor, box, label, votes, disabled = false, selected = false }) {
  ...
  const lineColor = disabled ? "#9CA3AF" : selected ? ACTIVE_SELECTION.stroke : "#111827";
  const cardFill = disabled ? "#F8FAFC" : selected ? ACTIVE_SELECTION.fill : "#FFFFFF";
  const cardStroke = disabled ? "#D7DEE8" : selected ? ACTIVE_SELECTION.stroke : "#D7DEE8";
  const titleColor = disabled ? "#64748B" : selected ? ACTIVE_SELECTION.title : "#111827";
  ...
  return (
    <g className="pointer-events-none select-none" opacity={disabled ? 0.52 : 1} style={{ userSelect: "none", WebkitUserSelect: "none" }}>
      ...
      {selected ? (
        <text x={box.x + CALLOUT_SIZE.width - 10} y={box.y + 19} textAnchor="end" style={{ fill: ACTIVE_SELECTION.stroke, fontSize: 10, fontWeight: 900 }}>
          선택
        </text>
      ) : null}
...
export default function SejongMap({
  ...
  const isAreaDisabled = (areaId) => {
    ...
    if (assignments?.[areaId]) return false;

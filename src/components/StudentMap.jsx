...
function RoundBadge({ missionConfig }) {
  return (
    <div className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-blue-900">
      <span className="rounded-lg bg-blue-600 px-2 py-1 text-xs font-black text-white">{missionConfig.round || 1}R</span>
      <span className="text-sm font-black">{missionConfig.name}</span>
    </div>
  );
}
...
function ActiveDistrictCard({ districtId, areaIds, electionDatasetId, populationRange, collapsed, onToggleCollapsed, onRemoveArea }) {
  const district = DISTRICT_THEME[districtId];
    ...
    <aside className={`absolute right-6 top-6 z-20 rounded-xl border border-slate-200 bg-white/95 shadow-xl backdrop-blur ${
      ...
      <div className="mt-3 rounded-lg bg-slate-50 px-3 py-2">
        ...
        <p className="mt-1 text-[11px] font-bold text-slate-500">
          기준 {formatNumber(populationRange.minPopulation)}~{formatNumber(populationRange.maxPopulation)}명 · {populationStatus}
        </p>
      </div>
      ...
      <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
        <div className="flex items-center justify-between gap-2">
          <p className="text-[10px] font-black uppercase tracking-[0.08em] text-slate-500">묶인 지역</p>
          <span className="text-[10px] font-black text-slate-400">눌러서 빼기</span>
        </div>
        {areaIds.length > 0 ? (
          <div className="mt-2 flex max-h-[112px] flex-wrap gap-1.5 overflow-y-auto pr-1">
            {areaIds.map((areaId) => (
              <button
                key={areaId}
                type="button"
                onClick={() => onRemoveArea(areaId)}
                className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[11px] font-black text-slate-700 transition hover:border-red-200 hover:bg-red-50 hover:text-red-600"
                title={`${AREA_BY_ID[areaId]?.name || areaId} 빼기`}
              >
                {AREA_BY_ID[areaId]?.name || areaId} ×
              </button>
            ))}
          </div>
        ) : (
          <p className="mt-2 rounded-lg bg-white px-2 py-2 text-center text-[11px] font-bold text-slate-400">아직 묶인 지역이 없습니다.</p>
        )}
      </div>
        </>
      )}
...
function CheckTab({ evaluation, connectedDb, populationRange, missionType }) {
  ...
  const populationViolationLabel = evaluation.populationViolations
    .map((result) => {
      const direction = result.population < populationRange.minPopulation ? "부족" : "초과";
      return `${districtName(result.districtId)} ${formatNumber(result.population)}명(${direction})`;
    })
    .join(", ");
  const round2Matched = seatsMatchRound2Target(evaluation.seats);
  const round2Ready = evaluation.canSubmit && evaluation.populationViolations.length === 0 && round2Matched;
    ...
    <>
      <div className="rounded-xl border border-slate-200 bg-slate-50">
        ...
        <div className="grid gap-1.5 p-3">
          ...
          {evaluation.populationViolations.length > 0 ? (
            <CheckItem state="warn">인구 기준을 벗어난 선거구: {populationViolationLabel}</CheckItem>
          ) : (
            <CheckItem state="ok">모든 선거구가 인구 기준을 만족합니다.</CheckItem>
          )}
        </div>
      </div>
      ...
      {missionType === "round2_extreme" ? (
        <div className="rounded-xl border border-purple-200 bg-purple-50">
          <div className="flex items-center justify-between border-b border-purple-100 px-3 py-2">
            <h2 className="text-sm font-black text-purple-950">2라운드 목표 확인</h2>
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-black ${round2Ready ? "bg-emerald-100 text-emerald-700" : "bg-white text-purple-800"}`}>
              {round2Ready ? "목표 달성" : "조정 필요"}
            </span>
          </div>
          <div className="grid gap-2 p-3 text-xs font-bold leading-5 text-purple-900">
            <p>현재 의석: 민주 {evaluation.seats.DEM || 0}석 · 국힘 {evaluation.seats.PPP || 0}석</p>
            <p>목표: 민주 4석·국힘 1석 또는 민주 1석·국힘 4석</p>
            <p>인구와 인접 조건을 지키면서 의석 조합을 극단적으로 바꿔보세요.</p>
          </div>
        </div>
      ) : null}

      {missionType === "round3_fair" ? (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50">
          <div className="flex items-center justify-between border-b border-emerald-100 px-3 py-2">
            <h2 className="text-sm font-black text-emerald-950">3라운드 공정성 기준</h2>
            <span className="rounded-full bg-white px-2 py-0.5 text-[10px] font-black text-emerald-800">
              비례성 {evaluation.proportionalityScore.toFixed(2)}
            </span>
          </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50">
        ...
        <div className="grid gap-1.5 p-3 text-xs font-bold leading-5 text-slate-500">
          ...
          <p>선거구 인구: {formatNumber(populationRange.minPopulation)}명~{formatNumber(populationRange.maxPopulation)}명</p>
...
export default function StudentMap({ pin, teamId, teamName = "우리 모둠", db }) {
  ...
  function removeAreaFromDistrict(areaId) {
    setAssignments((current) => ({
      ...current,
      [areaId]: null,
    }));
    setSelectedAreaIds((current) => current.filter((id) => id !== areaId));
    setMessage(`${AREA_BY_ID[areaId]?.name || "선택한 지역"}을 선거구에서 뺐습니다.`);
  }
    ...
    <main className="h-screen overflow-hidden bg-slate-50 text-slate-900">
      <header className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-4 border-b border-slate-200 bg-white px-5 py-2.5 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <RoundBadge missionConfig={missionConfig} />
        ...
        <section className="flex min-h-0 flex-col overflow-hidden bg-slate-100">
          ...
          <div className="flex items-center gap-2 border-b border-slate-200 bg-slate-50 px-4 py-2">
            ...
            <p className="hidden whitespace-nowrap text-xs font-extrabold text-slate-500 xl:block">
              {RECOMMENDED_DISTRICT_COUNT}개 선거구 · 기준 {formatNumber(populationRange.minPopulation)}~{formatNumber(populationRange.maxPopulation)}명
            </p>
            <button
              type="button"
          ...
          <div className="relative min-h-0 flex-1 overflow-auto">
            <div className="min-h-full p-3">
              <ActiveDistrictCard
                ...
                onRemoveArea={removeAreaFromDistrict}
              />
              <SejongMap

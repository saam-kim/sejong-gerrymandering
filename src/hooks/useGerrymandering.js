...
function getMissionSuccess(assignmentsEvaluation, missionType, targetSeats) {
  if (!assignmentsEvaluation.canSubmit || assignmentsEvaluation.populationViolations.length > 0) return false;
  if (missionType === "round2_extreme") return assignmentsEvaluation.missionSuccess;
  if (missionType === "round1_valid" || missionType === "round3_fair") return true;
  return assignmentsEvaluation.missionSuccess;
}
...
function isScoreEligible(entry, missionType) {
  ...
  return (
    ...
    entry.isValid &&
    Number(entry.violations?.population || 0) === 0
  );
}
...
export function useGerrymandering({
  ...
  useEffect(() => {
    ...
    const storageHandler = (event) => {
      if (event.key === FIREBASE_CONFIG_STORAGE_KEY) handler();
    };
    window.addEventListener("firebase-config-changed", handler);
    window.addEventListener("storage", storageHandler);
    return () => {
      window.removeEventListener("firebase-config-changed", handler);
  ...
  useEffect(() => {
    ...
    const unsubscribers = [
      ...
      onValue(ref(database, gamePath(pin, "roundProgress")), (snapshot) => {
        setRoundProgress(snapshot.val() || {});
      }),
    ];
  ...
  const submitPlan = useCallback(
    async (assignments) => {
      ...
      await set(ref(database, gamePath(pin, `submissions/${teamId}`)), submissionPayload);

      if (missionSuccess) {
        await update(ref(database, gamePath(pin, `roundProgress/${teamId}/${missionType}`)), {
          teamId,
          teamName: teamName || teamId,
          missionType,
          success: true,
